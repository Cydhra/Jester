...
/// might use a different scheme to generate a key) and encrypting and decrypting messages. No attempts for securing the
/// keys are made, it is the caller's responsibility to properly protect the keys from attacker's attempts to
/// retrieve them from memory according to the caller's threat model.
pub trait SymmetricalEncryptionScheme {
    type Key;

    /// Generate a random key from the key space of this algorithm.
    /// #Parameters
